...
interface DamageEvent {
  ...
  move: string; damage: number; crit: boolean; effectiveness: number;
  weather: string | null; terrain: string | null;
  screens: { reflect: boolean; lightScreen: boolean };
  attacker: { species: string; level: number; stat_atk: number; stat_def: number; stat_spa: number; ability: string; item: string; boosts: Record<string, number>; status: string | null; teraType: string | null; };
  defender: { species: string; level: number; stat_atk: number; stat_def: number; stat_spd: number; ability: string; item: string; boosts: Record<string, number>; status: string | null; hp_before: number; hp_after: number; max_hp: number; };
}
...
async function runBattle(scenario: typeof allScenarios[0]) {
  ...
  const chunks: string[] = [];
  const omniscientDone = (async () => { for await (const chunk of streams.omniscient) { chunks.push(chunk); } })();
  ...
  return { id: scenario.id, description: scenario.description, seed: scenario.seed, format: 'gen9customgame', p1: { name: 'Player 1', team: scenario.p1 }, p2: { name: 'Player 2', team: scenario.p2 }, events };
}

function parseOmniscientLog(log: string, scenario: typeof allScenarios[0]): DamageEvent[] {
  ...
  const pokemon: Record<string, { species: string; level: number; ability: string; item: string; boosts: Record<string, number>; status: string | null; hp: number; maxHp: number; stats: Record<string, number>; }> = {};
  ...
  for (const line of lines) {
    ...
    if (cmd === 'turn') { currentTurn = parseInt(parts[1]); }
      ...
      pokemon[slot] = { species, level, ability: teamMon?.ability || '', item: teamMon?.item || '', boosts: { atk: 0, def: 0, spa: 0, spd: 0, spe: 0 }, status: null, hp, maxHp, stats };
    } else if (cmd === 'move') {
      const source = parts[1].split(':')[0].trim();
      lastMove = { source, move: parts[2] };
    } else if (cmd === '-crit') { nextCrit = true; }
    else if (cmd === '-supereffective') { nextEffectiveness = 2.0; }
    else if (cmd === '-resisted') { nextEffectiveness = 0.5; }
    else if (cmd === '-damage') {
      ...
      if (damage > 0) {
        events.push({
          type: 'damage', turn: currentTurn, source: lastMove.source, target: targetSlot,
          move: lastMove.move, damage, crit: nextCrit, effectiveness: nextEffectiveness,
          weather, terrain,
          screens: targetSlot.startsWith('p2') ? { ...p2Screens } : { ...p1Screens },
          attacker: { species: attacker.species, level: attacker.level, stat_atk: attacker.stats.atk || 0, stat_def: attacker.stats.def || 0, stat_spa: attacker.stats.spa || 0, ability: attacker.ability, item: attacker.item, boosts: { ...attacker.boosts }, status: attacker.status, teraType: terastallized[lastMove.source] || null },
          defender: { species: target.species, level: target.level, stat_atk: target.stats.atk || 0, stat_def: target.stats.def || 0, stat_spd: target.stats.spd || 0, ability: target.ability, item: target.item, boosts: { ...target.boosts }, status: target.status, hp_before: hpBefore, hp_after: hpAfter, max_hp: target.maxHp },
        });
      }
      target.hp = hpAfter;
      nextCrit = false; nextEffectiveness = 1.0;
    } else if (cmd === '-boost') {
      const slot = parts[1].split(':')[0].trim();
      ...
      const slot = parts[1].split(':')[0].trim();
      if (pokemon[slot]) { pokemon[slot].hp = parseInt(parts[2].split(' ')[0].split('/')[0]); }
    } else if (cmd === '-enditem') {
      const slot = parts[1].split(':')[0].trim();
      ...
      if (effect === 'Reflect') side.reflect = true;
      else if (effect === 'Light Screen') side.lightScreen = true;
    } else if (cmd === '-sideend') {
      const side = parts[1].startsWith('p1') ? p1Screens : p2Screens;
      const effect = parts[2].replace('move: ', '');
      if (effect === 'Reflect') side.reflect = false;
      else if (effect === 'Light Screen') side.lightScreen = false;
...
async function main() {
  ...
  console.log(`Generating ${allScenarios.length} new batch fixtures...`);
  let total = 0;
  ...
  console.log(`Done. Total damage events: ${total}`);
}
